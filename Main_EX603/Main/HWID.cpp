...
void cHwid::SendHwid()
{
	...
	pMsg.AntihackDllCRC = GetFileCRC(gProtect.m_MainInfo.PluginName);
	pMsg.MainDllCRC = GetFileCRC("Main.dll");
	pMsg.MainExeCRC = GetFileCRC(gProtect.m_MainInfo.ClientName);
	pMsg.PlayerBmdCRC = GetFileCRC("Data/Player/player.bmd");
	pMsg.SkillBmdCRC = GetFileCRC("Data/Local/Eng/Skill_Eng.bmd");
	pMsg.ItemBmdCRC = GetFileCRC("Data/Local/Eng/Item_Eng.bmd");
