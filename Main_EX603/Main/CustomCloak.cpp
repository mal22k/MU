...
void cCustomCloak::LoadTexture() // OK
{
	for(int n=0;n < MAX_CUSTOMCLOAK;n++)
	{
		if(this->m_Cloak[n].Index != -1){
			...
			pLoadImage(this->m_Cloak[n].CloakName, this->m_Cloak[n].IndexCloak, GL_LINEAR, GL_REPEAT, 1, 0);

			if(this->m_Cloak[n].Mode == 1){

				pLoadImage(this->m_Cloak[n].StripName, this->m_Cloak[n].IndexStrip, GL_LINEAR, GL_REPEAT, 1, 0);
			}
		}
	}
}