...
//--
		...
		//gInterface.DrawItem(PosX+45,PosY-30,4,4,10,SET_ITEMOPT_LEVEL(0),0,0,0); si activo esto aparece un item arriba de la cabeza como un logo
		...
		switch(this->m_Data[index].m_Classe)
		{
			...
			pDrawBarForm((float)(PosX + 45), (float)(PosY + 14), 158.0f, 58.0f, 0.0f, 0);
			pGLSwitchBlend();
			this->DrawFormat(eGold180, PosX + 44, PosY + 17, 90, 3, "RANGO:  %s", this->m_Data[index].szTitleRank);
			this->DrawFormat(eWhite, PosX + 78, PosY + 30, 113, 3, " Level:  %d | Resets: %d ", this->m_Data[index].m_Level, this->m_Data[index].m_Resets);
			this->DrawFormat(eWhite, PosX + 87, PosY + 50, 100, 2,  "Kills:  %d  ", this->m_Data[index].m_Kills, this->m_Data[index].m_Deads); //se puede agregar : | Deaths: %d
			this->DrawFormat(eGold180, PosX + 143, PosY + 58, 120, 2, "LEGADO MU");
			/*this->DrawFormat(eGold180, PosX + 88, PosY + 16, 158, 3, "Legado Mu", this->m_Data[index].m_MResets);*/

			pDrawImage(gInterface.Data[this->m_Data[index].m_Rank + 1].ModelID, (float)PosX + 43, (float)PosY - -32, 60.0, 56.0, 0.0, 0.0, 1.0, 1.0, 1, 1, 0.0); //LOGO
...
void CCustomRankUser::GCReqRankLevelUser(PMSG_CUSTOM_RANKUSER * aRecv)
{
	...
	gObjUser.m_TodayDaily = aRecv->iTodayDaily;
#if FLAG
