...
void PrintAdvanceStatus(int x, int y)
{
	...
	v6 = 11;
	Width = 115;
	//gInterface.DrawFormat(eGold, JCResto + x - 185, 5, flWidth, 3, "ADVANCED STATUS");
	gInterface.DrawFormat(eGold, x + 48, 27, 170, 1, "ADVANCED STATUS");
...
void CharacterInfoWindow(int a1)
{
	...
	*(signed int *)(This + 16) = 450 + JCResto + Fix_DisplayWinExt;
	...
	if(pCheckMouseOver(x + 50, y + 53, 43, 17) )
	{
	...
	*(DWORD *)(MainSlot + 40) = Return2X(x + (Width / 2) - TransFor2X(t / 2));
	...
	if (gInterface.CheckWindow(ObjWindow::Inventory))
	{
		...
		else
		{
			...
			gInterface.Data[eTrash].OnShow = true;

			float MainWidth = 230.0;

			float MainHeight = 130.0;

			if (CheckButtonPressed(PosX + 440, PosY + 175, 43, 17))
			{
				gInterface.DrawGUI(eTrash, PosX + 440, PosY + 175); //aca se ajusta el boton
				RenderBitmap(61535, PosX + 440, PosY + 175, 43, 17, 0.0, 0.2235384732, 0.5035384717, 0.2215384692, 1, 1, 0);
			}
			else
			{
				RenderBitmap(61535, PosX + 440, PosY + 175, 43, 17, 0.0, 0.0, 0.5035384717, 0.2215384692, 1, 1, 0);
			}

			if (gInterface.IsWorkZone(eTrash))
			{
				//gInterface.DrawToolTip(PosX + 430, PosY + 165, "");
			}

			pSetTextColor(pTextThis(), 0xFFu, 0xFFu, 0xFFu, 0xFFu);
			ZeroMemory(v10, sizeof(v10));
			wsprintf(v10, pGetTextLine(pTextLineThis, 3841));// Advanced Status
			pDrawText(pTextThis(), PosX + 440, PosY + 178, "Clear", 43, 0, (LPINT)3, 0);
		}
	}
}
...
void JCNumberStack(float x, float y, int Number, float scale)
{
	...
	gInterface.CRenderNumberExBank(v24, v23, Number, ( pWinWidthReal <= 1.6 ) ? 1.0 : 0.90, 31337);
}
/*
