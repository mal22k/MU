...
void CCustomExchangeCoin::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.RequireRuud = lpMemScript->GetAsNumber();
			...
			info.RewardRuud = lpMemScript->GetAsNumber();
...
bool CCustomExchangeCoin::GetInfoByName(LPOBJ lpObj, char* message, int Npc) // OK
{
	...
	for(std::map<int,CUSTOMEXCHANGECOIN_INFO>::iterator it=this->m_CustomExchangeCoinInfo.begin();it != this->m_CustomExchangeCoinInfo.end();it++)
	{
		if(_stricmp(it->second.Name,command) == 0)
		{
			...
			if (it->second.RequireRuud > 0 && it->second.RequireRuud > lpObj->Ruud)
			{
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(891), it->second.RequireRuud);
				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj, Npc, gMessage.GetMessage(891), it->second.RequireRuud);
				}
				return 1;
			}
			...
			int Ruud = (((it->second.RewardRuud < 0) ? 0 : it->second.RewardRuud) - ((it->second.RequireRuud < 0) ? 0 : it->second.RequireRuud));

			GDSetCoinSend(lpObj->Index, Coin1, Coin2, Coin3,Ruud,"ExchangeCoin");
			return 1;
		}
	}
