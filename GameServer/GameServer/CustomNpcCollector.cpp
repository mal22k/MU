...
void CCustomNpcCollector::Load(char* path)
{
	...
	for(int n=0;n < MAX_ITEM_LIST;n++)
	{
		...
		this->m_Data[this->m_count].m_Ruud = 0;
	}
		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					this->m_Data[this->m_count].m_Ruud = lpMemScript->GetAsNumber();
	...
	if (Count == 64)
	{
		for(int i = 0; i < this->m_count; i++)
		{
			if(ItemIndex == GET_ITEM(this->m_Data[i].m_Cat,this->m_Data[i].m_Item))
			{
				...
				GDSetCoinSend(lpObj->Index, this->m_Data[i].m_Coin1, this->m_Data[i].m_Coin2, this->m_Data[i].m_Coin3, this->m_Data[i].m_Ruud,"NpCCollector");
