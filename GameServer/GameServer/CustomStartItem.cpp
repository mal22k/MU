		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					this->m_EnableInfo.push_back(info);
				}
				else if(section == 1)
				{
					if(strcmp("end",lpMemScript->GetAsString()) == 0)
...
{
	for(std::vector<ENABLE_INFO>::iterator it=this->m_EnableInfo.begin();it != this->m_EnableInfo.end();it++)
	{
...
void CCustomStartItem::InsertCoin(LPOBJ lpObj) // OK
{
	for(std::vector<STARTITEM_COIN_INFO>::iterator it=this->m_CustomStartItemCoinInfo.begin();it != this->m_CustomStartItemCoinInfo.end();it++)
	{
		...
		GDSetCoinSend(lpObj->Index, (it->Coin1 < 0)?0:it->Coin1, (it->Coin2 < 0)?0:it->Coin2, (it->Coin3 < 0)?0:it->Coin3, (it->Ruud < 0) ? 0 : it->Ruud,"StartItem");
	}
}

void CCustomStartItem::InsertStartStats(LPOBJ lpObj) // OK
{
