		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 3)
				{
					...
					this->m_ChaosCastleRewardValue[level][3] = lpMemScript->GetAsNumber();
				}
				else
				{
					break;
...
void CChaosCastle::GiveUserRewardItem(CHAOS_CASTLE_LEVEL* lpLevel,int aIndex) // OK
{
	...
	if (this->m_ChaosCastleRewardValue[lpLevel->Level][0] > 0 || this->m_ChaosCastleRewardValue[lpLevel->Level][1] > 0 || this->m_ChaosCastleRewardValue[lpLevel->Level][2] > 0 || this->m_ChaosCastleRewardValue[lpLevel->Level][3] > 0)
	{
		//Reward
		GDSetCoinSend(lpUser->Index, this->m_ChaosCastleRewardValue[lpLevel->Level][0], this->m_ChaosCastleRewardValue[lpLevel->Level][1], this->m_ChaosCastleRewardValue[lpLevel->Level][2], this->m_ChaosCastleRewardValue[lpLevel->Level][3],"CC");
...
void CChaosCastle::CGChaosCastleEnterRecv(PMSG_CHAOS_CASTLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCNewMessageSend(aIndex,0,0,gMessage.GetMessage(914),gServerInfo.m_ChaosCastleMaxEntryCount[lpObj->AccountLevel]);
		return;
	}
