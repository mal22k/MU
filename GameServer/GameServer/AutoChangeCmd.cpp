...
CAutoChangeCmd g_AutoChangeCmd;

CAutoChangeCmd::CAutoChangeCmd()
{
...
void CAutoChangeCmd::LoadFile(char* FilePath) // OK
{
		...
		while (true)
		{
			...
			while (true)
			{
				...
				else if (section == 1)
				{
					...
					AUTOCHANGE_ITEM_DATA m_ChangeData;
					...
					this->m_mapChangeData.insert(std::pair<int, AUTOCHANGE_ITEM_DATA>(m_ChangeData.ItemID, m_ChangeData));
...
int CAutoChangeCmd::AutoChange(int aIndex) // -1 - system off, 0 - no change made, > 0 - count of items changed
{
		...
		//	for (int i = INVENTORY_WEAR_SIZE; i < INVENTORY_MAIN_SIZE; i++)
	{
		...
		std::map<int, AUTOCHANGE_ITEM_DATA>::iterator It = this->m_mapChangeData.find(lpObj->Inventory[i].m_Index);
		...
		//오토환전 ll Auto Exchange

		if (It->second.CoinType == 0)
		{
			GDSetCoinSend(lpObj->Index, It->second.CoinValue, 0, 0,0, "Auto Exchange");
		}

		if (It->second.CoinType == 1)
		{
			GDSetCoinSend(lpObj->Index, 0, It->second.CoinValue, 0,0, "Auto Exchange");
		}

		if (It->second.CoinType == 2)
		{
			GDSetCoinSend(lpObj->Index, 0, 0, It->second.CoinValue,0, "Auto Exchange");
		}
