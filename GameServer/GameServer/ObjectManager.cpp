...
#if(BATTLE_PASS)
	gCustomBattlePass.OnMonsterKill(lpObj, lpMonster);
#endif
...
#if(EXT_MASTER_TREE)
		gCustomExtMasterTree.OnMasterLevelUp(lpObj);
#endif
...
#if(EXT_MASTER_TREE)
	gCustomExtMasterTree.CalcExtMasterOptions(lpObj, true);
#endif
...
#if(BATTLE_PASS)
	gCustomBattlePass.GDBattlePassLoad(aIndex);
#endif

#if(EXT_MASTER_TREE)
	gCustomExtMasterTree.GDExtMasterLoad(aIndex);
#endif
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			if(gObj[SummonIndex].Type == OBJECT_USER)
			{
				gCustomKillStreak.OnPlayerKill(&gObj[SummonIndex], lpTarget);
				gCustomKillStreak.OnPlayerDeath(lpTarget);
			}
#endif
			...
			if(gObj[SummonIndex].Type == OBJECT_USER)
			{
				gCustomBattlePass.OnPlayerKill(&gObj[SummonIndex], lpTarget);
			}
#endif
