		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 4)
				{
					...
					this->m_DevilSquareRewardValue[level][3] = lpMemScript->GetAsNumber();
				}
				else
				{
					break;
...
void CDevilSquare::GiveUserRewardMoney(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_DS_USER;n++)
	{
		...
		{
			if (this->m_DevilSquareRewardValue[lpLevel->Level][0] > 0 || this->m_DevilSquareRewardValue[lpLevel->Level][1] > 0 || this->m_DevilSquareRewardValue[lpLevel->Level][2] > 0 || this->m_DevilSquareRewardValue[lpLevel->Level][3] > 0)
			{
				//Reward
				GDSetCoinSend(lpLevel->User[n].Index, this->m_DevilSquareRewardValue[lpLevel->Level][0], this->m_DevilSquareRewardValue[lpLevel->Level][1], this->m_DevilSquareRewardValue[lpLevel->Level][2], this->m_DevilSquareRewardValue[lpLevel->Level][3],"DS");
...
void CDevilSquare::CGDevilSquareEnterRecv(PMSG_DEVIL_SQUARE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCNewMessageSend(aIndex,0,0,gMessage.GetMessage(915),gServerInfo.m_DevilSquareMaxEntryCount[lpObj->AccountLevel]);
		return;
	}
