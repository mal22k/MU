...
void CChaosBox::ChaosItemMix(LPOBJ lpObj) // OK
{
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		...
		if (gItemBagManager.GetItemBySpecialValue(ITEM_BAG_SPECIAL_CHAOS_ITEM_MIX, lpObj, &item) == 0)
		{
...
void CChaosBox::Wing2Mix(LPOBJ lpObj, int type) // OK
{
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		if (type == 0)
		{
			...
			if (gItemBagManager.GetItemBySpecialValue(ITEM_BAG_SPECIAL_CHAOS_WING_2_MIX, lpObj, &item) == 0)
			{
				this->GCChaosMixSend(lpObj->Index, 0, 0);
				return;
			}
		...
		else
		{
			...
			if (gItemBagManager.GetItemBySpecialValue(ITEM_BAG_SPECIAL_CHAOS_CAPE_MIX, lpObj, &item) == 0)
			{
				this->GCChaosMixSend(lpObj->Index, 0, 0);
				return;
			}
...
void CChaosBox::Wing1Mix(LPOBJ lpObj) // OK
{
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		...
		if (gItemBagManager.GetItemBySpecialValue(ITEM_BAG_SPECIAL_CHAOS_WING_1_MIX, lpObj, &item) == 0)
		{
			this->GCChaosMixSend(lpObj->Index, 0, 0);
			return;
		}
...
void CChaosBox::Wing3Mix(LPOBJ lpObj) // OK
{
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		...
		if (gItemBagManager.GetItemBySpecialValue(ITEM_BAG_SPECIAL_CHAOS_WING_3_MIX, lpObj, &item) == 0)
		{
			this->GCChaosMixSend(lpObj->Index, 0, 0);
			return;
		}
...
void CChaosBox::CGChaosMixRateRecv(PMSG_CHAOS_MIX_RATE_RECV* lpMsg, int aIndex) // OK
{
	...
	default:
	{
		...
		if (gCustomMix.CheckItem(lpObj, lpMsg->type) == 0)
		{
			return;
		}
		...
		pMsg.rate = gBonusManager.GetBonusValue(lpObj, BONUS_INDEX_CHAOSMIX_RATE, lpMsg->type);
