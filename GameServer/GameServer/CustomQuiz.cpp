		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 2)
				{
					...
					info_question.Value4 = lpMemScript->GetAsNumber();
...
void CCustomQuiz::CommandQuiz(LPOBJ lpObj,char* arg)   
{
	...
	if (this->GetInfo(this->IndexSelected,&CustomQuiz) == 1)
	{
		if(strcmp(_strlwr(arg),_strlwr(CustomQuiz.Answer)) == 0)
		{
			...
			//-- Reward
			GDSetCoinSend(lpObj->Index, CustomQuiz.Value1, CustomQuiz.Value2, CustomQuiz.Value3, CustomQuiz.Value4,"CustomQuiz");
