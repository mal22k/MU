		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 3)
				{
					...
					this->Ruud = lpMemScript->GetAsNumber();
				}
				else
				{
					break;
	...
	for(int n=0;n < MAX_TVTEVENT_USER;n++)
	{
		if(OBJECT_RANGE(this->User[n].Index) != 0)
		{
			...
			if(this->User[n].Team == TeamReward)
			{
				...
				if (this->Coin1 > 0 || this->Coin2 > 0 || this->Coin3 > 0 || this->Ruud > 0)
				{	
					GDSetCoinSend(lpObj->Index, this->Coin1, this->Coin2, this->Coin3, this->Ruud,"TvTEvent");
				}
