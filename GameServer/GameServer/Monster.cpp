...
void gObjMonsterDieGiveItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->Class == 247 || lpObj->Class == 249 || lpObj->Class == 277 || lpObj->Class == 283 || lpObj->Class == 288 || lpObj->Class == 460 || lpObj->Class == 461 || lpObj->Class == 462 || lpObj->Class == 524 || lpObj->Class == 525 || lpObj->Class == 527 || lpObj->Class == 528 || lpObj->Class == 605 || lpObj->Class == 606 || lpObj->Class == 607 || gCustomGuard.GetGuard(lpObj->Class))
	{
		return;
	}
	...
	if(lpObj->Level >= 1 && (GetLargeRand()%1000000) < SetItemDropRate)
	{
...
bool gObjSetMonster(int aIndex,int MonsterClass) // OK
{
	...
	if(lpObj->AttackType != 0)
	{
		...
		CUSTOM_MONSTER_SKILL* MonsterAttackSkill = gCustomMonsterSkill.GetSkillMonster(MonsterClass);
...
bool gObjMonsterMoveCheck(LPOBJ lpObj,int tx,int ty) // OK
{
	...
	else
	{
		...
		if (gMonsterSetBase.m_MonsterSetBaseInfo[lpObj->PosNum].Type == 3 || gMonsterSetBase.m_MonsterSetBaseInfo[lpObj->PosNum].Type == 4)
		{
			if (lpObj->ActionState.Emotion == 1) //invasions,events allows to follow target unlimited
			{
				return 1;
			}
		}
...
void gObjMonsterAttack(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	else if(lpObj->Class == 529 || lpObj->Class == 530 || lpObj->Class == 533 || lpObj->Class == 534 || lpObj->Class == 535 || lpObj->Class == 536 || lpObj->Class == 537 || lpObj->Class == 538 || lpObj->Class == 539 || gCustomMonsterSkill.GetSkillMonster(lpObj->Class))
	{
...
BOOL gObjMonsterGetTargetPos(LPOBJ lpObj)
{
	...
	if ( gMap[lpObj->Map].CheckStandAttr(tpx, tpy) == 0 )
	{
		while ( searchcount-- )
		{
			...
			if ( lpObj->Class == 249 || gCustomGuard.GetGuard(lpObj->Class))
			{
...
void gObjMonsterMoveAction(LPOBJ lpObj)
{
	...
	while ( searchc-- != 0 )
	{
		...
		if ( (lpObj->Class ==249 || gCustomGuard.GetGuard(lpObj->Class) || lpObj->Class == 247) && mchk )
		{
			if ( (attr&2)!=2)
			{
				lpObj->MTX = tpx;
...
void gObjMonsterBaseAct(LPOBJ lpObj)
{
	...
	if ( lpObj->ActionState.Emotion == 0 )
	{
		if ( lpObj->Attribute )
		{
			...
			if (lpObj->Class == 249 || lpObj->Class == 247 || gCustomGuard.GetGuard(lpObj->Class))
			{
