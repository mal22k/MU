...
void CCustomMonsterKillCount::DGMonsterKillCountRecv(SDHP_MONSTER_KILL_COUNT_RECV* lpMsg) // OK
{
	...
	for(std::map<int,MONSTER_KILL_COUNT_INFO>::iterator it=this->m_MonsterKillCountInfo.begin();it != this->m_MonsterKillCountInfo.end();it++)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"%s %d/%d",gMonsterManager.GetMonsterName(lpMsg->MonsterClass),lpMsg->Count,it->second.KillCount);
			continue;
		}
		...
		{
			gCashShop.GDCashShopAddPointSaveSend(lpObj->Index,0,it->second.Coin1,it->second.Coin2,it->second.Coin3, it->second.Ruud);
			gCashShop.CGCashShopPointRecv(lpObj->Index);

			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(629),it->second.Coin1,it->second.Coin2,it->second.Coin3, it->second.Ruud);
		}
