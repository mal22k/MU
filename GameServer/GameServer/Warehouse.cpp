...
void CWarehouse::DGWarehouseItemRecv(SDHP_WAREHOUSE_ITEM_RECV* lpMsg) // OK
{
	...
	lpObj->WarehouseQtd = lpMsg->WarehouseCount;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
bool CWarehouse::WarehouseChangeRecv(PMSG_WAREHOUSE_CHANGE_RECV* lpMsg, int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	if (lpMsg->BauID < 0 || lpMsg->BauID > gServerInfo.m_CommandWareNumber[lpObj->AccountLevel])
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(87));
		return false;
	}

	GDInventoryItemSaveSend(lpObj->Index);

	gWarehouse.GDWarehouseItemSaveSend(lpObj->Index);
	// ---
	if (lpObj->Interface.type == INTERFACE_WAREHOUSE)
	{
		lpObj->WarehouseMoney = 0;
		...
		lpObj->WarehouseNumber = lpMsg->BauID;

		lpObj->Interface.use = 1;

		lpObj->Interface.state = 0;

		lpObj->WarehouseState = 1;
...
void CWarehouse::GCReqWarehouseUserCountSend(LPOBJ lpObj)
{
	PMSG_WAREHOUSEINFO_SEND pMsg;

	pMsg.header.set(0xFB, 0xD5, sizeof(pMsg));
	// ---
	pMsg.BauID = lpObj->WarehouseNumber;

	if (lpObj->WarehouseQtd < gServerInfo.m_CommandWareNumber[lpObj->AccountLevel])
	{
		pMsg.BauQtd = gServerInfo.m_CommandWareNumber[lpObj->AccountLevel];
	}
	else
	{
		pMsg.BauQtd = lpObj->WarehouseQtd;
	}

	pMsg.BauLock = gServerInfo.m_CommandWareType;
	// ---
	DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size);
}