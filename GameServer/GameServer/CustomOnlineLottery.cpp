		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 2)
				{
					...
					this->m_CustomOnlineLotteryInfo[index].Reward.Ruud = lpMemScript->GetAsNumber();
...
void CCustomOnlineLottery::ProcState_START(CUSTOM_ONLINE_LOTTERY_INFO* lpInfo) // OK
{
	...
	if(lpInfo->RemainTime <= 0)
	{
					...
					if (index > 0 )
					{
						...
						int Ruud = this->m_CustomOnlineLotteryInfo[lpInfo->Index].Reward.Ruud;


						if (Coin1 > 0 || Coin2 > 0 || Coin3 > 0 || Ruud > 0)
						{	
							GDSetCoinSend(lpObj->Index, Coin1, Coin2, Coin3, Ruud, "Lottery");
						}
					}
					else
					{
...
void CCustomOnlineLottery::StartNow(int map) // OK
{
	int Map = map;
					...
					if (index > 0 )
					{
						...
						int Ruud = this->m_CustomOnlineLotteryInfo[0].Reward.Ruud;
