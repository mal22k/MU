		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 3)
				{
					...
					this->m_IllusionTempleReward[level][3] = lpMemScript->GetAsNumber();
				}
				else
				{
					break;
...
void CIllusionTemple::CGIllusionTempleEnterRecv(PMSG_ILLUSION_TEMPLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCNewMessageSend(aIndex,0,0,gMessage.GetMessage(917),gServerInfo.m_IllusionTempleMaxEntryCount[lpObj->AccountLevel]);
		return;
	}
...
void CIllusionTemple::CGIllusionTempleButtonClickRecv(int aIndex) // OK
{
	...
	if (this->m_IllusionTempleReward[level][0] > 0 || this->m_IllusionTempleReward[level][1] > 0 || this->m_IllusionTempleReward[level][2] > 0 || this->m_IllusionTempleReward[level][3] > 0)
	{
		//Reward
		GDSetCoinSend(lpUser->Index, this->m_IllusionTempleReward[level][0], this->m_IllusionTempleReward[level][1], this->m_IllusionTempleReward[level][2], this->m_IllusionTempleReward[level][3],"IT");
